                ...
                if (pos1++) {
                    ...
                    if (*pp == 'k') {
                        if (strncmp(pp, "keystate_numlock", 16))
                            if (strncmp(pos1 - 1, "enable", 6)) gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(Flag_NumLock), TRUE);

                        if (strncmp(pp, "keystate_capslock", 17))
                            if (strncmp(pos1 - 1, "enable", 6)) gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(Flag_CapsLock), TRUE);

                        if (strncmp(pp, "keystate_scrolllock", 19))
                            if (strncmp(pos1 - 1, "enable", 6)) gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(Flag_ScrollLock), TRUE);
                    }
                }
            }
        }
...
void  save_file(char file_out[]) {
    ...
    else fprintf(f, "#keystate_numlock = enable\n");

    if (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(Flag_ScrollLock))) fprintf(f, "keystate_scrolllock = enable\n");
    else fprintf(f, "#keystate_scrolllock = enable\n");

    if (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(Flag_CapsLock))) fprintf(f, "keystate_capslock = enable\n");
    else fprintf(f, "#keystate_capslock = enable\n");
